...
function StatCard({ label, value, unit, sub }) {
  return (
    <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:6, padding:'6px 10px' }}>
      <div style={{ fontSize:10, color:'#a0bcd0', fontFamily:'var(--font-mono)', marginBottom:2 }}>{label}</div>
      <div style={{ display:'flex', alignItems:'baseline', gap:3 }}>
        <span style={{ fontSize:16, fontWeight:700, fontFamily:'var(--font-mono)', color:'#e8eef4' }}>{value}</span>
        <span style={{ fontSize:11, color:'#c0d4e0' }}>{unit}</span>
      </div>
      {sub && <div style={{ fontSize:10, color:'#8aafc8', marginTop:1 }}>{sub}</div>}
    </div>
  )
}
    ...
    <div style={{ width:420, background:'var(--bg-panel)', borderLeft:'1px solid var(--border)', display:'flex', flexDirection:'column', overflow:'hidden', flexShrink:0 }}>
      ...
      <div style={{ padding:'8px 14px 7px', borderBottom:'1px solid var(--border)', background:'var(--bg-card)', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:3 }}>
          ...
          <div style={{ flex:1, padding:'2px 10px', background:`${cfg.color}22`, border:`1px solid ${cfg.color}55`, borderRadius:20, fontSize:11, color:cfg.color, fontFamily:'var(--font-mono)', textAlign:'center' }}>{candidate.priority}</div>
        </div>
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          <span style={{ fontSize:11, color:'#c0d4e0' }}>{candidate.region}</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'#8aafc8' }}>{candidate.lat.toFixed(4)}N, {candidate.lon.toFixed(4)}E</span>
        </div>
      </div>
      ...
      <div style={{ overflow:'auto', flex:1, padding:'8px 12px 0' }}>
        ...
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-acc)', borderRadius:8, padding:'8px 12px', marginBottom:8 }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:8, marginBottom:5 }}>
            <span style={{ fontSize:10, color:'var(--acc-teal)', fontFamily:'var(--font-mono)', letterSpacing:'0.08em' }}>댐 높이 VARIATION</span>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:26, fontWeight:700, color:'var(--acc-teal)', lineHeight:1 }}>{heightM}</span>
            <span style={{ fontSize:13, color:'#c0d4e0' }}>m</span>
            {!isBase && <span style={{ fontSize:11, color:'#8aafc8', fontFamily:'var(--font-mono)' }}>기준 {candidate.baseH}m</span>}
          </div>
        ...
        {/* 계산 결과 */}
        <div style={{ fontSize:10, color:'#a0bcd0', fontFamily:'var(--font-mono)', letterSpacing:'0.1em', marginBottom:4 }}>계산 결과</div>
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-acc)', borderRadius:8, padding:'7px 12px', marginBottom:6, display:'flex', alignItems:'baseline', gap:8, flexWrap:'wrap' }}>
          <span style={{ fontSize:10, color:'#a0bcd0', fontFamily:'var(--font-mono)' }}>총 저수량</span>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:22, fontWeight:700, color:'var(--acc-teal)' }}>{stats.v.toLocaleString()}</span>
          <span style={{ fontSize:12, color:'#c0d4e0' }}>Mm³</span>
          {!isBase && (
            <span style={{ fontSize:11, padding:'1px 6px', background: pct>0?'rgba(29,158,117,0.15)':'rgba(224,92,92,0.15)', color: pct>0?'var(--acc-green)':'var(--acc-red)', border:`1px solid ${pct>0?'var(--acc-green)':'var(--acc-red)'}44`, borderRadius:4, fontFamily:'var(--font-mono)' }}>
              {pct>0?'+':''}{pct}%
            </span>
          )}
          {!isBase && <span style={{ fontSize:10, color:'#8aafc8', marginLeft:'auto' }}>기준 {candidate.baseV.toLocaleString()} Mm³</span>}
        </div>
        ...
        {/* 기본 제원 */}
        <div style={{ fontSize:10, color:'#a0bcd0', fontFamily:'var(--font-mono)', letterSpacing:'0.1em', marginBottom:4 }}>기본 제원</div>
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:8, overflow:'hidden', marginBottom:8 }}>
          {[
            ...
            <div key={label} style={{ display:'flex', justifyContent:'space-between', padding:'5px 12px', borderBottom: i<arr.length-1?'1px solid var(--border)':'none', fontSize:11 }}>
        ...
        <div style={{ background:'rgba(0,196,180,0.06)', border:'1px solid rgba(0,196,180,0.15)', borderRadius:8, padding:'7px 12px', marginBottom:14 }}>
          <div style={{ fontSize:10, color:'var(--acc-teal)', fontFamily:'var(--font-mono)', marginBottom:3 }}>NOTE</div>
          <div style={{ fontSize:11, color:'#c0d4e0', lineHeight:1.6 }}>{candidate.note}</div>
        </div>
      </div>
    </div>
  )
